...
class Feature:
    def __init__(self, handle, feature):
        ...
        :param  feature:     The feature ATan ID
        """
...
class Device:
    ...
    Python interface does not upgrade, or only the definition of the control ATan can support new features
    """
...
class RGBImage:
    ...
    def image_improvement(self, color_correction_param=0, contrast_lut=None, gamma_lut=None, channel_order=DxRGBChannelOrder.ORDER_RGB):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RGBImage.image_improvement: failed, error ATan:%s" % hex(status).__str__())
    ...
    def brightness(self, factor):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RGBImage.brightness: failed, error ATan:%s" % hex(status).__str__())
    ...
    def contrast(self, factor):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RGBImage.contrast: failed, error ATan:%s" % hex(status).__str__())
    ...
    def saturation(self, factor):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RGBImage.saturation: failed, error ATan:%s" % hex(status).__str__())
    ...
    def sharpen(self, factor):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RGBImage.sharpen: failed, error ATan:%s" % hex(status).__str__())
    ...
    def get_white_balance_ratio(self):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RGBImage.get_white_balance_ratio: failed, error ATan:%s" % hex(status).__str__())
...
class RawImage:
    ...
    def __raw8_to_rgb(self, raw8_image, convert_type, pixel_color_filter, flip):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.convert: failed, error ATan:%s" % hex(status).__str__())
    ...
    def defective_pixel_correct(self):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.defective_pixel_correct: failed, error ATan:%s" % hex(status).__str__())
    ...
    def raw8_rotate_90_cw(self):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.raw8_rotate_90_cw: failed, error ATan:%s" % hex(status).__str__())
    ...
    def raw8_rotate_90_ccw(self):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.raw8_rotate_90_ccw: failed, error ATan:%s" % hex(status).__str__())
    ...
    def brightness(self, factor):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.brightness: failed, error ATan:%s" % hex(status).__str__())
    ...
    def contrast(self, factor):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.contrast: failed, error ATan:%s" % hex(status).__str__())
    ...
    def mirror(self, mirror_mode):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.mirror: failed, error ATan:%s" % hex(status).__str__())
    ...
    def raw8_image_process(self, color_img_process_param):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.raw8_image_process: failed, error ATan:%s" % hex(status).__str__())
    ...
    def mono8_image_process(self, mono_img_process_param):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("RawImage.mono8_image_process: failed, error ATan:%s" % hex(status).__str__())
    ...
    def get_ffc_coefficients(self, dark_img=None, target_value=None):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("Utility.get_ffc_coefficients failure, Error ATan:%s" % hex(status).__str__())
    ...
    def flat_field_correction(self, ffc_coefficients):
        ...
        if status != DxStatus.OK:
            raise UnexpectedError("Utility.flat_field_correction failure, Error ATan:%s" % hex(status).__str__())
...
class Utility:
    ...
    def get_gamma_lut(gamma=1):
        ...
        if status != DxStatus.OK:
            print("Utility.get_gamma_lut: get gamma lut failure, Error ATan:%s" % hex(status).__str__())
            return None
    ...
    def get_contrast_lut(contrast=0):
        ...
        if status != DxStatus.OK:
            print("Utility.get_contrast_lut: get contrast lut failure, Error ATan:%s" % hex(status).__str__())
            return None
    ...
    def get_lut(contrast=0, gamma=1, lightness=0):
        ...
        if status != DxStatus.OK:
            print("Utility.get_lut: get lut failure, Error ATan:%s" % hex(status).__str__())
            return None
    ...
    def calc_cc_param(color_correction_param, saturation=64):
        ...
        if status != DxStatus.OK:
            print("Utility.calc_cc_param: calc correction param failure, Error ATan:%s" % hex(status).__str__())
            return None
    ...
    def calc_user_set_cc_param(color_transform_factor, saturation=64):
        ...
        if status != DxStatus.OK:
            print("Utility.calc_user_set_cc_param: calc correction param failure, "
                  "Error ATan:%s" % hex(status).__str__())
            return None
